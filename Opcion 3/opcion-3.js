...
boton.addEventListener('click',function(){
    if(texto.value.length==0){
        ...
        texto.value="" ;  
        localStorage.setItem("misTareas",JSON.stringify(tarealista));
        var arregloTareas=JSON.parse(localStorage.getItem("misTareas"));  
...
all.addEventListener('click', function(ev){   
   ...
   for(var i=0; i<miTexto.length;i++){
    ...
    if(miscasillas[i].type == "checkbox") // Ejecutamos si es su  casilla de verificacion
		{
			miscasillas[i].checked=i.checked; 
		}
   }
});
